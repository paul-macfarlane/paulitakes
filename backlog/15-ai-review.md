...
- [~] **AIR-1** — Record the agreed editorial, agent-auth, proposal and collaboration contracts in the product/design docs and ADR. _(deps: none)_
- [~] **AIR-2** — Safe concurrent saves on direct and staged drafts, opaque edit versions, conflict recovery preserving all unsaved fields, and two-editor regression coverage (FR-7.11). Includes the foundation formerly called COLLAB-1; collaboration reuses it. _(deps: AIR-1)_
- [ ] **AIR-3** — Immutable proposal snapshots, metadata/body diffs, structured editorial notes and guarded selective apply; one open AI proposal per post (FR-7.12). _(deps: AIR-2)_
- [ ] **AIR-4** — Mobile-first human review UI with per-change selection, full comparison/preview and separate fact-check/media notes (FR-7.12). _(deps: AIR-3)_
