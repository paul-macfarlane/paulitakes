...
- **FR-7.13** — Agent access is independently authenticated, scoped, revocable and auditable; it exposes only review content and proposal creation. No sessions, account data, comments, analytics, direct content writes or publishing capabilities. Local Codex is the first client. _(Server API: AIR-5; local Codex adapter: AIR-6.)_
