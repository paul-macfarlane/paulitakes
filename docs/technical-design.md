...
AIR-4 supplies the human review screen. AIR-3 exposes owner/admin read/apply/reject server actions, but no agent creation endpoint: the server-only submission service accepts a trusted principal to be supplied by AIR-5 authentication. It validates strict candidate/notes/skill inputs and logs only fixed operation labels on errors, never database parameters or draft text. Skill attribution documents the supplied brief; it does not certify compliance or factual correctness.
