...
AIR-5 implements the scoped REST API; AIR-6 adds a one-shot command-line helper for local Codex reviews. The [local client runbook](runbooks/agent-client.md) and [ADR-0035](adr/0035-command-line-editor-client.md) define its four commands, configured origin/token/skill path and exact installed-brief attribution. No MCP service or registration is needed; submission validates the hash returned by the explicit brief-loading step. The request-only [editor workflow](harness/workflows/agent-review.md) explicitly loads the maintained skill each review; no editorial brief is copied into the application. Only proposal creation may be an agent POST; the existing server-action rule continues for human post/lifecycle mutations. Use a dedicated configured token and sanitized audit patterns from Picksleagues, with fixed review-read/proposal-create capabilities and revocation through token replacement/removal and server restart/redeployment (ADR-0032). Agent access includes all authors' reviewable content, never human admin authority or user/comment/analytics data.
