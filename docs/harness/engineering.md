...
- **Presentation-only PRs include visual evidence.** When a PR is strictly visual — it changes UI presentation without changing behavior, API/data contracts, auth, persistence, or business rules — capture the affected route and meaningful changed state at **390px** and **1024px** in the applicable theme(s), then attach or link the screenshots in the PR for review. Capture the smallest representative set that makes the change reviewable; do not dump unrelated routes or take screenshots for nonvisual work. Use synthetic/local data only, never cookies, tokens, or PII. Playwright is preferred for authenticated states (`docs/harness/workflows/verify.md`); a focused browser capture is fine for a public route. If screenshots cannot be produced, say why in the PR and describe the visual verification performed, so a reviewer does not mistake missing evidence for an unreviewed layout.
- **Review screenshots are temporary artifacts, not repository assets.** Keep capture scripts in Git, but write generated screenshots to an ignored `test-results/review-screenshots/` directory. Publish only the smallest reviewed set as a GitHub Actions artifact with `retention-days: 7`, and link the artifact, captured commit, and expiry from the PR. Never commit review images or force-add an ignored capture directory unless the owner explicitly requests a permanent reference: deleting images in a later commit does not remove their bytes from Git history. Verify the uploaded images and link, then remove local output; regenerate expired evidence when review needs it. If artifact upload is unavailable, share local previews with the owner and state the limitation rather than committing images as a workaround. Keep any screenshot CI opt-in for the affected UI, not every PR. Product assets and explicitly maintained visual-test baselines are separate from review evidence.
